  ...
  D: Deserializer<'de>,
{
  ...
  String::deserialize(deserializer).and_then(|string| {
    u128::from_str_radix(&string, 16).map_err(|_| Error::invalid_value(Unexpected::Str(&string), &"hex encoded token"))
  })
}
            ...
            PalaceMessage::StartGame(message) => {
              ...
              if let Some(lobby) = lobbies.get_mut(&message.lobby_id) {
                ...
                if lobby.players.len() < 2 {
                  return self
                    .out
                    .send(serde_json::to_vec("can't start a game with less than 2 players").unwrap());
                }
